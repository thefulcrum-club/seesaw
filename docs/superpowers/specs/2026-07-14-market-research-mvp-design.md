...
- **AI (all pipeline agents):** Gemini 3 Flash Preview (`gemini-3-flash-preview`), server-side only, key in `.env.local`/Vercel env. Model history during implementation: started on `gemini-2.5-flash` (Task 7 hit its 20 req/day free-tier quota), switched to `gemini-2.5-flash-lite` (also hit its own 20 req/day quota shortly after), switched to `gemini-3-flash-preview` (separate quota bucket as a distinct model, confirmed working live post-Task-15-blocker). Being a preview model, availability/quota/behavior may change upstream without notice — revisit if it becomes unstable. Search grounding is used per-stage in place of a standalone web-search tool. JSON schema mode (`responseSchema`) is used for structured writes into `research_state`.
