...
- **AI (all pipeline agents):** Gemini Flash-Lite (`gemini-2.5-flash-lite`), server-side only, key in `.env.local`/Vercel env. Switched from `gemini-2.5-flash` during implementation (Task 7) because the free-tier daily quota for full Flash (20 req/day) was blocking live testing of the multi-agent pipeline; Flash-Lite has a materially higher free-tier cap while still supporting Search grounding and `responseSchema`. Search grounding is used per-stage in place of a standalone web-search tool. JSON schema mode (`responseSchema`) is used for structured writes into `research_state`.
