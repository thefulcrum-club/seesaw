...
- **AI (all pipeline agents):** Gemini 2.5 Flash (`gemini-2.5-flash`), called via **Vertex AI** (not the plain Gemini API key) — `src/lib/gemini/client.ts` uses `GoogleGenAI({ vertexai: true, project, location })` with Application Default Credentials (`gcloud auth application-default login`) when `GOOGLE_CLOUD_PROJECT` is set, falling back to a plain `GEMINI_API_KEY` otherwise. Model/auth history during implementation: the Gemini API key's free tier caps at 20 requests/day *per model*, and this was hit three times in a row across three different models (`gemini-2.5-flash` → `gemini-2.5-flash-lite` → `gemini-3-flash-preview`, the last of which also isn't published on Vertex AI in this project/region). Switching to Vertex AI (billed against the GCP project, not the API-key free tier) resolved this — confirmed by running the full live test suite successfully (all 7 agent test files, including the full pipeline). If reverting to a plain API key, expect the same 20 req/day-per-model ceiling to reappear. Search grounding is used per-stage in place of a standalone web-search tool. JSON schema mode (`responseSchema`) is used for structured writes into `research_state`.
